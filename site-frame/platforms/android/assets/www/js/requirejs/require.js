...
(function(ba) {
	...
	function ga(b) {
		function c(a, k, b) {
			...
			if (a) {
				...
				l = a;
				for (c = 0; c < l.length; c++) if (d = l[c], "." === d) l.splice(c, 1), c -= 1;
	...
	if ("undefined" === typeof define) {
		...
		if (z && (y = x.head = document.getElementsByTagName("rjs")[0], D = document.getElementsByTagName("base")[0])) y = x.head = D.parentNode;
		g.onError = ca;
		...
		z && !q.skipDataMain && T(document.getElementsByTagName("script"), function(b) {
			y || (y = b.parentNode);
			if (I = b.getAttribute("rjs-config")) return s = I, q.baseUrl || (E = s.split("/"), s = E.pop(), O = E.length ? E.join("/") + "/" : "./", q.baseUrl =
